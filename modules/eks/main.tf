...
module "eks_blueprints" {
  ...
  count = var.self_managed_node_groups_enabled != null ? 1 : 0
...
#---------------------------------------------------------------
# EKS Managed Blueprints
#---------------------------------------------------------------

module "eks_blueprints_managed" {
  source = "git::https://github.com/aws-ia/terraform-aws-eks-blueprints.git?ref=v4.21.0"
  count = var.managed_node_groups_enabled != null ? 1 : 0
  ...
  #----------------------------------------------------------------------------------------------------------#
  cluster_security_group_additional_rules = {
    ingress_bastion_to_managed_cluster = {
      # name        = "allow bastion ingress to cluster"
      description              = "Bastion SG to Cluster"
      security_group_id        = module.eks_blueprints.cluster_security_group_id
      from_port                = 443
      ...
      protocol                 = "tcp"
      type                     = "ingress"
      source_security_group_id = var.source_security_group_id_managed
    }
  }
  ...
  managed_node_groups = {
    ...
    managed_ng1 = {
      ...
      create_iam_role           = false                                         # Changing `create_iam_role=false` to bring your own IAM Role
      iam_role_arn              = aws_iam_role.managed_ng.arn              # custom IAM role for aws-auth mapping; used when create_iam_role = false
      iam_instance_profile_name = aws_iam_instance_profile.managed_ng.name # IAM instance profile name for Launch templates; used when create_iam_role = false
  ...
  #   }
  #   managed_ng5 = {
  #     node_group_name = "managed_ng5" # Name is used to create a dedicated IAM role for each node group and adds to AWS-AUTH config map
...
resource "aws_iam_role" "auth_eks_role_managed" {
