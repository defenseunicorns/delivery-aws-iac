...
module "tfstate_backend" {
  source = "git::https://github.com/defenseunicorns/terraform-aws-tfstate-backend"
  ...
  cluster_key_admin_arns = [
    "arn:${data.aws_partition.current.partition}:iam::${var.account}:user/${var.aws_admin_1_username}",
    "arn:${data.aws_partition.current.partition}:iam::${var.account}:user/${var.aws_admin_2_username}"
  ]
}
