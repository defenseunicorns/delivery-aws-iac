...
module "flux_sops" {
  ...
  region                     = var.region
  cluster_name               = module.eks-managed.eks_cluster_id
  vpc_id                     = module.vpc.vpc_id
  policy_name_prefix         = "${module.eks-managed.eks_cluster_id}-flux-sops"
  kms_key_alias              = "${module.eks-managed.eks_cluster_id}-flux-sops"
  kubernetes_service_account = "flux-system-sops-sa"
  kubernetes_namespace       = "flux-system"
  irsa_sops_iam_role_name    = "${module.eks-managed.eks_cluster_id}-flux-system-sa-role"
  eks_oidc_provider_arn      = module.eks-managed.eks_oidc_provider_arn
  tags                       = local.tags
...
module "loki_s3_bucket" {
  ...
  irsa_iam_role_name         = "${module.eks-managed.eks_cluster_id}-logging-loki-sa-role"
  eks_oidc_provider_arn      = module.eks-managed.eks_oidc_provider_arn
  tags                       = local.tags
